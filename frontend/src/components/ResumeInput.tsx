...
export function ResumeInput({
  ...
  async function handleFileChange(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      onUploadedResumeChange(text);
      setUploadedFileName(file.name);
      setUploadSaveMessage(null);
  ...
  async function handleSaveUploadedResume() {
    if (!uploadedResume.trim()) {
      setUploadSaveMessage("Upload a resume file before saving.");
      return;
    }
        ...
        <div className="space-y-3">
          <div className="rounded-lg border border-dashed border-gray-300 bg-gray-50 p-6 text-center">
            ...
            <button
              ...
              className="mt-3 rounded-lg bg-white px-4 py-2 text-sm font-medium text-indigo-700 ring-1 ring-inset ring-indigo-200 hover:bg-indigo-50 disabled:opacity-50"
            >
              {uploadedResume.trim() ? "Choose different file" : "Choose file"}
            </button>
            {uploadError && (
          ...
          {uploadedResume.trim() && (
            <div className="space-y-3">
              ...
              <div className="flex flex-wrap items-center gap-3">
                <button
                  type="button"
                  disabled={disabled || isUploadSaving}
                  onClick={() => void handleSaveUploadedResume()}
                  className="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700 disabled:opacity-50"
                >
                  {isUploadSaving ? "Saving..." : "Save resume"}
                </button>
              </div>
              {uploadSaveMessage && (
                <p
                  className={`text-sm ${
                    uploadSaveMessage.startsWith("Failed") ||
                    uploadSaveMessage.startsWith("Upload a resume")
                      ? "text-red-600"
                      : "text-gray-600"
                  }`}
                  role="status"
